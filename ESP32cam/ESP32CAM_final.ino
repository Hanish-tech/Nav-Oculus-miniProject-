...
//輸入WIFI連線帳號密碼
const char* ssid     = "Storm breaker 2.0";   //your network SSID
const char* password = "pikachu@22";   //your network password
...
void loop() {
  ...
  int commandIndex = 0;  // Create an integer variable to store the command index
  // Map the strings to integer indices
  if (serial == "PEDESTRIAN") {
      commandIndex = 1;
  } else if (serial == "GO") {
      commandIndex = 2;
  } else if (serial == "STOP") {
      commandIndex = 3;
  } else if (serial == "U_TURN") {
      commandIndex = 4;
  } else if (serial == "NULL") {
      commandIndex = 5;
  } else if (serial == "RIGHT_TURN") {
      commandIndex = 6;
  } 
